...
def createParser():
    ...
    parser.add_argument('-i', '--intmethod', dest='method', nargs=1, type=str, default='SINC',
            help='Interpolation method one of SINC, BILINEAR, BICUBIC, NEAREST, AKIMA, BIQUINTIC')
...
@use_api
def resampSlave(burst, offdir, outname, sn, azpoly, rgpoly, method,
...
#            doppler = slave._dopplerVsPixel
    ...
    resampSlave(slave, inps.offsets, outfile, inps.slave, azpoly, rgpoly, inps.method, flatten=(not inps.noflat), zero=inps.zero, dims = inps.dims, master = master)
