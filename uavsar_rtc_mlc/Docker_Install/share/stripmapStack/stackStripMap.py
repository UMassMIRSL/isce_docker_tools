...
def get_dates(inps):
    ...
    for dirf in dirs:
        fn2 = [f for f in os.listdir(os.path.join(dirf)) if f.endswith('.slc')]
        if inps.nofocus:
            expectedRaw = os.path.join(dirf,fn2[0])
            #if os.path.isfile(os.path.join(dirf,fn2[0])):
    ...
    #acuisitionDates.extend([os.path.split(f)[1] for f in dirs])
...
def main(iargs=None):
  ...
  if not os.path.exists(runDir):
       os.makedirs(runDir)
  if inps.sensor.lower() == 'uavsar_stack': 
